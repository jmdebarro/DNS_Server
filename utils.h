...
char *extract_domain_from_query(char buffer[BUFF_LENGTH]) {
    unsigned int index = QUERY_OFFSET;
    while (buffer[i] != 0) {
        unsigned int string_len = buffer[i];
        buffer[i + string_len] = '.';
        i += string_len;
    }

    return (char *)(buffer + QUERY_OFFSET + 1);
}
...
ssize_t modify_blocked_domain_buffer(char buffer[BUFF_LENGTH], ssize_t bytes_received) {
